...
const BattleArena = () => {
  ...
  const handleAttack = async (moveIndex) => {
    try {
      const response = await fetch('https://projectbackend-ypns.onrender.com/combat/attack', {
        method: 'POST',
        headers: {
  ...
  const handleChangePokemon = async (pokemonName, forcedChange = false) => {
    try {
      ...
      const response = await fetch('https://projectbackend-ypns.onrender.com/combat/change', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${getToken()}`,
  ...
  useEffect(() => {
    const deleteCombat = async () => {
      if (combatState.winner) {
        try {
          const response = await fetch('https://projectbackend-ypns.onrender.com/combat', {
            method: 'DELETE',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${getToken()}`,
